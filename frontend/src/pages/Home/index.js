...
function Home() {
  ...
  const [indige, setIndige] = useState({ data: "" });
...
function CreateSidebar({
  ...
  const [toglleActionstate, setToglleActionstate] = useState(1);
  ...
  const handleChangeSelect = async (e) => {
    ...
    //   setIfwason(false)
    //  }
    if (e.target.name === "rowChild") {
      setChildinfo({ ...childinfo, subnav: e.target.value });
      console.log(e.target.value);
    }
    console.log(e.target.value);
    if (e.target.name === "floor") {
      setIndige({ data: e.target.value });
      console.log(sidebarSelected, "this ");
      setChildinfo({ ...childinfo, parent: e.target.value });
    }
    if (e.target.name === "captionChild") {
      setChildinfo({ ...childinfo, caption: e.target.value });
    }
  ...
  const onHandleChange = async (e) => {
    e.preventDefault();
    try {
      console.log(toglleActionstate)
      // sendingAlldata([row,floor,caption])
      if (toglleActionstate === 0) {
        await axios
          .post(
            `http://localhost:8001/api/v2/user/createsidebarchild`,
            childinfo
          )
  ...
  const toggleAction = async (e) => {
    ...
    if (e.nativeEvent.target.selectedIndex === 0) {
      setToglleActionstate(0);
    }
    if (e.nativeEvent.target.selectedIndex === 1) {
      setToglleActionstate(1);
    }
    if (e.nativeEvent.target.selectedIndex === 2) {
      setToglleActionstate(2);
    }
    // setToglleActionstate
    // if (e.target.name === "updata") {
    //   setToglleActionstate({...childinfo,subnav:e.target.value});
    // console.log(e.target.value)
    // }
    // console.log(e.target.value);
    // if (e.target.name === "create") {
    //   setToglleActionstate({...childinfo,parent:e.target.value});
    // }
    // if (e.target.name === "delete") {
    //   setToglleActionstate({...childinfo,caption:e.target.value});
        ...
        <div>
          <form onSubmit={onsubmitSidebar}>
            <div className="SidebarAction">
              {/*  update childs */}
              <select onClick={toggleAction}>
                <option name="update" value="updata">
                  updata sub navigaton
                </option>
                <option name="create" value="create">
                  create sub navigaton
                </option>
                <option name="delete" value="delete">
                  delete sub navigaton
                </option>
              </select>
              <label htmlFor="floor">بخش زير مجموعه</label>

              {/*  __________ UPDATE SUB NAVIGATION ____________________________________________________________ */}

              {toglleActionstate === 0 && (
                <div className="hdskljfhsdfkl">
                  <h1 className="hsfkhslfkj">تغير دادن</h1>
                  <select
                    defaultValue="2"
                    name="floor"
                    type="number"
                  ...
                  <input
                    ...
                    onChange={handleChangeSelect}
                  />
                  <button onClick={onHandleChange}>ارسال</button>
                </div>
              )}

              {/*  __________ END UPDATE SUB NAVIGATION ____________________________________________________________ */}

              {/*   CREATE SUB NAVIGATION ____________________________________________________________ */}

              {toglleActionstate === 1 && (
                <div className="hdskljfhsdfkl">
                  <h1 className="hsfkhslfkj">اضافه كردن</h1>
                  <select
                    defaultValue="2"
                    name="floor"
                    type="number"
              ...
              {/* ____________END  CREATE SUB NAVIGATION ____________________________________________________________ */}

              {/*  __________ DELETE SUB NAVIGATION ____________________________________________________________ */}

              {toglleActionstate === 2 && (
                <div className="hdskljfhsdfkl">
                  <h1 className="hsfkhslfkj">هذف كردن </h1>
                  <select
                    defaultValue="2"
                    name="floor"
                    type="number"
